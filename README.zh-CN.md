...
实时识别优先根据句末标点另起一段。如果模型持续输出没有标点的长文本，
FreeCommunication 会在稳定的 `36` 词或 `240` 字符上限处另起一段。这个过程只拆分
显示段落，不会删减转录内容。
...
1. 选择通话、视频或现场模式。
2. 点击**开始**，等待“加载模型”提示变为实时流式状态。模型预热完成后才正式开始捕捉
   音频。
3. 录制途中可随时开关翻译。已经生成的译文会保留，新内容会在纯英文和中英双行布局间
   平滑切换。
4. 通话模式可随时开关麦克风。
5. 可以即时复制当前原文、译文或原文加译文。
6. 需要减少遮挡时进入字幕模式。
7. 点击**结束**。系统会刷新最后一段流式缓存，然后直接写入转录与音频，不会在结束后
   重新识别完整会议。
